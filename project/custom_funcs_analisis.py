...
def plot_minmax(*montos_dicts, labels=None):
    ...
    plt.bar(r2, mean_values, color='orange', width=bar_width, edgecolor='orange', label='Mean')
...
def plot_totals(*montos_dicts, labels=None):
    ...
    for bar, label, total in zip(bars, labels, totals):
        ...
        plt.text(bar.get_x() + bar.get_width() / 2, height, f' ${formatted_total}', 
...
def Monto(dataset, grupo, region, Reverse):
    ...
    for regiones in region:
        total = round((dataset.loc[dataset[grupo] == regiones, 'MONTO FEDERAL'].sum()), 2)
        estados_total[regiones] = total/1000000

    #formato de mayor a menor
    estados_total = dict(sorted(estados_total.items(), key=lambda item: item[1], reverse=Reverse))    
    return estados_total

def Entidades_beneficiarios(dataset,figsize):
    Estados = Entidades(dataset=dataset,columna='ENTIDAD')
    Estados_Montos = Monto(dataset=dataset, grupo='ENTIDAD',region=Estados, Reverse=True)
