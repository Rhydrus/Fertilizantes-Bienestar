    ...
    for folder in os.listdir(mexico_shp_path):
        ...
        if os.path.isdir(folder_path):  # Verifica si es una carpeta
            for archivo in os.listdir(folder_path):
                if archivo.endswith('.shp'):  # Verifica si el archivo tiene la extensión .shp
...
def Beneficiarios(year=2020):
    ...
    if year == 2021:
        df = pd.read_excel('data\\raw\\listado_beneficiarios_fertilizantes_2021.xlsx', engine='openpyxl')
    elif year == 2022:
        df = pd.read_excel('data\\raw\\listado_beneficiarios_fertilizantes_2022.xlsx', engine='openpyxl')
    elif year == 2023:
        df = pd.read_excel('data\\raw\\listado_beneficiarios_fertilizantes_2023.xlsx', engine='openpyxl')
    else:
        df = pd.read_excel('data\\raw\\listado_beneficiarios_fertilizantes_2020.xlsx', engine='openpyxl')
